...
/**
 * 第二个clone函数，第一个有问题，有死循环
 * @author QuinnPan<pjkui@qq.com>
 * @param {object} src被复制的对象
 * @param {boolean} 是否进行深复制、
 * @return {object} 返回复制对象
 */
jmUtils.clone=function(src, deep) {
 ...
 * @return {bound} 当前控件边界
 */
jmSequence.prototype.getBounds = function () {
    var rect = {};
    this.initPoints();
    var p = this.getLocation();
    rect.left = p.left;
 ...
 * @return {boolean} 如果在则返回true,否则返回false
 */
jmSequence.prototype.checkPoint = function (p) {
    //生成当前坐标对应的父级元素的相对位置
    var abounds = this.bounds || this.getBounds();
 ...
 * @private
 */
jmSequence.prototype.initPoints = function () {
    var location = this.getLocation();
    var p1 = {x: location.left, y: location.top};//左下
    var p2 = {x: location.left + location.width, y: location.top};//右下
    var p3 = {x: location.left + location.width, y: location.top + location.height};//左上
    var p4 = {x: location.left, y: location.top + location.height};//右上
...
function jmCell(option) {
    ...
    if (option.preConditions) {
        //precess the precondition
        var preCndLng = option.preConditions.length;
        if (preCndLng > 0) {
            for (var i = 0; i < preCndLng; ++i) {
                this.addPrecondition(option.preConditions[i].value);
            }
        }
    }

    //precess the post condition on the cell
    if (option.postConditions) {
        var postCndLng = option.postConditions.length;
        if (postCndLng > 0) {
            for (var i = 0; i < postCndLng; ++i) {
                this.addPostcondition(option.postConditions[i].value);
            }
        }
    }
}
 ...
 */
jmCell.prototype.addPrecondition = function (value_) {

    //根据preCondition的数量来确定当前的插入位置
    var yPosition = (this.preConditions.length + 1) * 6;
    var preline = this.graph.createShape('line', {
        start: {x: 2, y: yPosition},
        end: {x: 98, y: yPosition},
        style: {
            stroke: '#6C54F3',
            normalStroke: 'black',
            overStroke: 'red',
    ...
    preline.visible = true;
    value_ = value_ || "preData_" + this.preConditions.length;
    preline.value = value_;
    this.children.add(preline);
    this.preConditions.push(preline);
}
 ...
 */
jmCell.prototype.addPostcondition = function (value_) {

    console.log("add post condition");
    var yPosition = this.height() - (this.postConditions.length + 1) * 6;
    var postline = this.graph.createShape('line', {
        start: {x: 2, y: yPosition},
        end: {x: 98, y: yPosition},
        style: {
            stroke: '#33E',
    ...
    postline.visible = true;
    value_ = value_ || "postData_" + this.postConditions.length;
    postline.value = value_;
    this.children.add(postline);
    this.postConditions.push(postline);
}
/**
 * 添加当前元素到画布中
 *
...
jmEditor.prototype.initEvents = function () {
    ...
    jmUtils.bindEvent(document, 'mouseleave', function (evt) {
        //console.log("mouse leave: jmEditor.js");
    });
    //鼠标移动事件，
    //有拖出组件时，移动组件
...
jmEditor.prototype.regComponent = function (el, option) {
    ...
    jmUtils.bindEvent(el, 'mousedown', function (evt) {
        ...
        var _this = self;
        evt = evt || event;
        var target = evt.target || evt.srcElement;//获得点击的目标
        var pos = jmUtils.getElementPosition(target);//获得点击的位置


        if (!target.getAttribute('data-component')) {
            //如果点击的元素没有data-component 的属性，并且没有子节点，就选择他的父节点
            if (target.children.length == 0) {
                target = target.parentElement;
            }
        }

        //用当前点击的图片给currentComponent赋值
        ...
        _this.currentComponent.img.style['position'] = 'fixed';

        _this.currentComponent.left = pos.left;
        _this.currentComponent.top = pos.top;
        console.log("mouse down position: left: " + pos.left + "  top:" + pos.top);
